...
function draw() {
    var colorDark = "#071820", colorLight = "#344c57", colorBackground = "#ecf4cb";
    ...
    fallingPiece.show();
    if (keyIsDown(DOWN_ARROW)) updateEvery = 2;
    else updateEvery = updateEveryCurrent;
    if (!pauseGame) {
        ticks++;
    ...
    for(let i = 0; i < gridPieces.length; i++)gridPieces[i].show();
    for(let i = 0; i < lineFades.length; i++)lineFades[i].show();
    if (gridWorkers.length > 0) gridWorkers[0].work();
...
function keyPressed() {
    if (!pauseGame) {
        if (keyCode === LEFT_ARROW) fallingPiece.input(LEFT_ARROW);
        else if (keyCode === RIGHT_ARROW) fallingPiece.input(RIGHT_ARROW);
        if (keyCode === UP_ARROW) fallingPiece.input(UP_ARROW);
    }
}
function playPiece() {
    ...
    this.nextPiece = function() {
        ...
        var xx = 525, yy = 365;
        if (this.nextPieceType != 0 && this.nextPieceType != 3) xx += gridSpace * 0.5;
    ...
    this.addPos = function(x, y) {
        this.pos.x += x;
        this.pos.y += y;
        if (this.pieces) for(var i = 0; i < 4; i++){
    ...
    this.futureCollision = function(x, y, rotation) {
        var xx, yy, points = 0;
        if (rotation != this.rotation) //Gets a new point orientation to check against
        ...
        for(var i = 0; i < this.pieces.length; i++){
            ...
            for(var j = 0; j < gridPieces.length; j++)if (xx === gridPieces[j].pos.x) {
                if (yy >= gridPieces[j].pos.y && yy < gridPieces[j].pos.y + gridSpace) return true;
                if (yy + gridSpace > gridPieces[j].pos.y && yy + gridSpace <= gridPieces[j].pos.y + gridSpace) return true;
            }
        }
    };
    //Handles input ;)
    this.input = function(key) {
        switch(key){
            case LEFT_ARROW:
                if (!this.futureCollision(-gridSpace, 0, this.rotation)) this.addPos(-gridSpace, 0);
                break;
            case RIGHT_ARROW:
                if (!this.futureCollision(gridSpace, 0, this.rotation)) this.addPos(gridSpace, 0);
                break;
            case UP_ARROW:
                ...
                if (!this.futureCollision(gridSpace, 0, rotation)) this.rotate();
                break;
        }
    };
    ...
    this.show = function() {
        for(var i = 0; i < this.pieces.length; i++)this.pieces[i].show();
        for(var i = 0; i < this.nextPieces.length; i++)this.nextPieces[i].show();
    };
    //Add the pieces to the gridPieces
    this.commitShape = function() {
        for(var i = 0; i < this.pieces.length; i++)gridPieces.push(this.pieces[i]);
        this.resetPiece();
        analyzeGrid();
    };
}
function square(x, y, type) {
    ...
    this.show = function() {
        strokeWeight(2);
        var colorDark = "#092e1d", colorMid = colors[this.type];
        fill(colorMid);
        stroke(25);
...
function pseudoRandom(previous) {
    ...
    if (roll === previous || roll === 7) roll = Math.floor(Math.random() * 7);
    return roll;
}
...
function analyzeGrid() {
    ...
    while(checkLines()){
        ...
        if (linesCleared % 10 === 0) {
            currentLevel += 1;
            //Increase speed here
            if (updateEveryCurrent > 4) updateEveryCurrent -= 1;
        }
    }
...
function checkLines() {
    ...
    for(var i = 0; i < gridPieces.length; i++)if (gridPieces[i].pos.y === runningY) {
        count++;
        if (count === 10) {
            //YEEHAW
...
function worker(y, amount) {
    ...
    this.work = function() {
        if (this.amountActual < this.amountTotal) {
            for(var j = 0; j < gridPieces.length; j++)if (gridPieces[j].pos.y < y) gridPieces[j].pos.y += 5;
            this.amountActual += 5;
        } else gridWorkers.shift();
    };
}
...
//# sourceMappingURL=index.ffcfb65a.js.map
