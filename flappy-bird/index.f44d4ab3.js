...
var flappy_bird = {
    ...
    update: function() {
        if (this.falls) {
            if (this.flashAnim > 255) this.flashReturn = true;
            if (this.flashReturn) this.flashAnim -= 60;
            else this.flashAnim += 60;
            ...
            if (!this.fly) this.velocityY += 0.4;
            else this.y -= 5;
            if (this.y > height - 49) {
                if (!flappy_bird.falls) try {
                    sound_hit.play();
                } catch (e) {
                    console.log(e);
                }
...
function setup() {
    if (mobile()) c = createCanvas(windowWidth, windowHeight);
    else c = createCanvas(400, 600);
    c.parent("FlappyBird");
    imageMode(CENTER);
    rectMode(CENTER);
    ellipseMode(CENTER);
...
function page_game() {
    overflowX += speed;
    if (overflowX > sprite_city.width / 2) overflowX = 0;
    //City
    ...
    if (!flappy_bird.falls) {
        if (parseInt(frameCount) % 70 === 0) pipes.push(new Pipe());
    }
...
function Pipe() {
    ...
    this.display = function() {
        ...
        //Pipes collisions
        if (flappy_bird.x + 20 > this.x - 25 && flappy_bird.x - 20 < this.x + 25 && flappy_bird.y + 20 > this.y - this.gapSize - sprite_pipe.height / 2 / 2 - 200 && flappy_bird.y - 20 < this.y - this.gapSize - sprite_pipe.height / 2 / 2 + 200 || flappy_bird.x + 20 > this.x - 25 && flappy_bird.x - 20 < this.x + 25 && flappy_bird.y + 20 > this.y + this.gapSize + sprite_pipe.height / 2 / 2 - 200 && flappy_bird.y - 20 < this.y + this.gapSize + sprite_pipe.height / 2 / 2 + 200) {
            if (!flappy_bird.falls) try {
                sound_hit.play();
            } catch (e) {
                console.log(e);
...
var menu_gameover = {
    ...
    display: function() {
        ...
        pop();
        if (press("restart", 0, 140, width / 2, height / 2)) resetGame();
...
function press(txt, x, y, tX, tY) {
    var this_h = false;
    if (mouseX > tX + x - textWidth(txt) / 2 - 10 && mouseX < tX + x + textWidth(txt) / 2 + 10 && mouseY > tY + y - textAscent() / 2 - 10 && mouseY < tY + y + textAscent() / 2 + 10) this_h = true;
    push();
    textSize(16);
    ...
    pop();
    if (this_h && mouseReleaseEvent) try {
...
//# sourceMappingURL=index.f44d4ab3.js.map
