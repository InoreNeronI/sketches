class Grid {
    ...
    createGrid() {
        for(let i = 0; i < this.w; i++){
            let col = [];
            for(let j = 0; j < this.h; j++)col.push(new Cell(i, j, this.cellSize));
            this.grid.push(col);
        }
        let lastRow = [];
        for(let i = 0; i < this.w; i++)if (i > this.w * 0.55) lastRow.push(true);
        else lastRow.push(false);
        shuffle(lastRow, true);
        for(let i = 0; i < this.w; i++)this.grid[i][this.h - 1].dead = lastRow[i];
    }
    update() {}
    validate() {
        let numberSelected = 0;
        for(let i = 0; i < this.w; i++){
            for(let j = 0; j < this.h; j++)if (this.grid[i][j].selected) numberSelected++;
        }
        if (numberSelected != 4) return false;
        for(let i = 0; i < targets.length; i++)if (this.gridContains(targets[i].shape)) {
    ...
    gridContains(shape) {
        ...
        dance: for(let j = 0; j < this.h; j++){
            for(let i = 0; i < this.w; i++)if (this.grid[i][j].selected) {
                firstGridCell = [
                    i,
                    j
                ];
        ...
        for(let i = 0; i < shape[0].length; i++){
            for(let j = 0; j < shape.length; j++)if (shape[i][j]) {
                targetShapeCount++;
                if (firstTargetCell == -1) firstTargetCell = [
                    j,
                    i
                ];
                else {
                    ...
                    if (cell instanceof Cell && !cell.selected) return false;
                }
                ...
                if (cellBelow instanceof Cell && !cellBelow.selected && !cellBelow.dead) return false;
    ...
    dropSelected() {
        for(let i = 0; i < this.w; i++){
            for(let j = 0; j < this.h; j++)if (this.grid[i][j].selected) {
    ...
    addNewRow() {
        for(let i = 0; i < this.w; i++){
            for(let j = 0; j < this.h; j++)if (this.grid[i][j].dead) {
    ...
    hover() {
        ...
        for(let i = 0; i < this.w; i++){
            for(let j = 0; j < this.h; j++)if (this.grid[i][j].hover()) {
                if (numberSelected >= 4 && !this.grid[i][j].selected) return false;
                else {
    ...
    clearSelected() {
        for(let i = 0; i < this.w; i++){
            for(let j = 0; j < this.h; j++)if (this.grid[i][j].selected) this.grid[i][j].selected = false;
        }
    }
    updateScore() {
        let actualScore = 0;
        for(let i = 0; i < this.w; i++){
            for(let j = 0; j < this.h; j++)if (!this.grid[i][j].dead) actualScore++;
        }
        if (score == -1 || actualScore < score) score = actualScore;
    }
    display() {
        push();
        ...
        for(let i = 0; i < this.w; i++)for(let j = 0; j < this.h; j++)this.grid[i][j].display();
        pop();
    }
}

//# sourceMappingURL=index.6703a2cc.js.map
