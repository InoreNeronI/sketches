 ...
 */ const ShortestPath = ()=>{
    ...
    sp.improvisePath = (state)=>{
        ...
        const neighbors = [
            state.direction
        ].concat(game.DIRECTIONS.filter((d)=>!d.eq(state.direction))).map((d)=>state.snake[0].sum(d)).filter((n)=>n.inBounds(state.nx, state.ny) && !state.snake.some((n1)=>n1.eq(n) && !state.walls.has(n.x, n.y)));
        if (neighbors.length > 0) return fnShortestPath.buildPath([
            start,
            neighbors[0]
        ]);
        else return fnShortestPath.buildPath([
            start,
...
const fnShortestPath = {
    search: (origin, goal, nx, ny, exclusions = NodeSet(), startDir = game.NORTH)=>{
        ...
        while(open.length > 0){
            ...
            if (current[1].eq(goal)) return fnShortestPath.buildPath(current);
                ...
                const dirs = [
                    currentDir
                ].concat(game.DIRECTIONS.filter((d)=>!d.eq(currentDir)));
                dirs.map((d)=>current[1].sum(d)).filter((n)=>n.inBounds(nx, ny) && !closed.has(n.x, n.y)).forEach((n)=>{
                    open.push([
                        current,
                        n
                    ]);
    ...
    buildPath: (current)=>{
        let path = [];
        for(let c = current; c != null; c = c[0])path.push(c[1]);
        return path;
    },
...
//# sourceMappingURL=index.cff84fbc.js.map
