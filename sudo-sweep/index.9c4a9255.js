class Minesweeper {
    constructor(x, y, w, h, subw, subh, font){
        ...
        this.grid;
        this.visibility = [
            ...Array(this.size * this.subw)
        ].map((e)=>Array(this.size * this.subw));
        this.flagged = [
            ...Array(this.size * this.subw)
        ].map((e)=>Array(this.size * this.subw));
        this.exploded = false;
    ...
    createMines(n) {
        ...
        let places = [];
        for(let i = 0; i < this.subsize; i++)if (i < n) places.push("\u2055");
        else places.push("");
        return shuffle(places);
    }
    placeMines(grid, mines, x, y) {
        for(let i = 0; i < this.subw; i++)for(let j = 0; j < this.subh; j++){
    ...
    getClues(grid) {
        for(let i = 0; i < this.size * this.subw; i++)for(let j = 0; j < this.size * this.subh; j++){
            if (grid[i][j] === "\u2055") continue;
            const neighbours = this.getNeighbours(grid, i, j);
            if (neighbours != 0) grid[i][j] = neighbours;
        }
        return grid;
    }
    getNeighbours(grid, x, y) {
        ...
        for(let i = x - 1; i <= x + 1; i++)for(let j = y - 1; j <= y + 1; j++){
            if (i === x && j === y) continue;
            if (i < 0 || i >= this.size * this.subw || j < 0 || j >= this.size * this.subh) continue;
    ...
    draw(x, y) {
        ...
        for(let i = 0; i < this.size * this.subw; i++)for(let j = 0; j < this.size * this.subh; j++){
            ...
            if (this.visibility[i][j] && !(this.flagged[i][j] && number === "\u2055")) fill("#DBEBE9");
            else fill(white);
            stroke(dark);
            strokeWeight(1);
            ...
            if (this.flagged[i][j] && !(this.visibility[i][j] && number != "\u2055")) {
                ...
                if (number === "\u2055") text("*", x + cellSize / 2, y + 60 / this.subh / 2 + 2);
                else text(number, x + cellSize / 2, y + 60 / this.subh / 2 + 2);
            }
        }
            ...
            if (x > i * cellSize && x < i * cellSize + cellSize) {
                if (y > j * 60 / this.subh && y < j * 60 / this.subh + 60 / this.subh) {
                    ...
                    if (mouseButton === LEFT && !this.flagged[i][j]) {
                        ...
                        } else this.visibility[i][j] = true;
                    } else if (mouseButton != LEFT && !this.visibility[i][j]) this.flagged[i][j] = !this.flagged[i][j];
                    if (mouseButton === LEFT) this.chord(i, j);
                }
            }
        }
    }
    ...
    freeNeighbours(x, y) {
        for(let i = x - 1; i <= x + 1; i++)for(let j = y - 1; j <= y + 1; j++){
            if (i === x && j === y) continue;
            else if (i < 0 || j < 0 || i >= this.size * this.subw || j >= this.size * this.subh) continue;
            if (!this.visibility[i][j]) {
                this.visibility[i][j] = true;
                if (this.grid[i][j] === "") this.freeNeighbours(i, j);
            }
        }
    }
    chord(i, j) {
        if (this.flaggedNeighbours(i, j) === this.grid[i][j]) this.chordNeighbours(i, j);
    }
    flaggedNeighbours(x, y) {
        ...
        for(let i = x - 1; i <= x + 1; i++)for(let j = y - 1; j <= y + 1; j++){
            if (i === x && j === y) continue;
            if (i < 0 || i >= this.size * this.subw || j < 0 || j >= this.size * this.subh) continue;
            if (this.flagged[i][j] && !this.visibility[i][j]) neighbours += 1;
        }
        return neighbours;
    }
    chordNeighbours(x, y) {
        for(let i = x - 1; i <= x + 1; i++){
            for(let j = y - 1; j <= y + 1; j++)if (i >= 0 && j >= 0 && i < this.size * this.subw && j < this.size * this.subh) {
                if (this.flagged[i][j]) ;
                else if (this.grid[i][j] === "\u2055") this.explode();
                else if (this.grid[i][j] === "") {
                    this.visibility[i][j] = true;
                    this.freeNeighbours(i, j);
                } else this.visibility[i][j] = true;
    ...
    validate() {
        ...
        for(let i = 0; i < this.size * this.subw; i++)for(let j = 0; j < this.size * this.subh; j++){
            if (!this.visibility[i][j] && this.grid[i][j] !== "\u2055") return false;
        }
        return true;
    }
}

//# sourceMappingURL=index.9c4a9255.js.map
