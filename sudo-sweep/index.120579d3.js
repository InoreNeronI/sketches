class Sudoku {
    ...
    findPath(grid, position, current) {
        if (position === this.size * this.size) return grid;
        if (this.conflict(grid, position, current)) return false;
        const x = this.getColumn(position);
        const y = this.getRow(position);
        grid[x][y] = current;
        position++;
        let options;
        if (this.size === 4) options = [
            1,
            2,
            3,
            4
        ];
        else if (this.size === 6) options = [
            1,
            2,
            3,
            4,
    ...
    conflict(grid, position, current) {
        ...
        for(let i = 0; i < this.size; i++){
            if (grid[column][i] === current || grid[i][row] === current) return true;
        }
        for(let i = 0; i < position; i++)if (sector === this.getSector(i)) {
            const iColumn = this.getColumn(i);
            const iRow = this.getRow(i);
            if (grid[iColumn][iRow] === current) return true;
        }
        return false;
    }
    getSector(position) {
        ...
        for(let i = 0; i < this.size; i++)for(let j = 0; j < this.size; j++){
            if (x > i * cellSize && x < i * cellSize + cellSize) {
                if (y > j * cellSize && y < j * cellSize + cellSize) {
                    ...
                    let max;
                    if (this.size === 4) max = 5;
                    else if (this.size === 6) max = 7;
                    else if (this.size === 9) max = 10;
                    if (!this.hint[i][j] && mouseButton === LEFT) {
                        this.guess[i][j] += 1;
                        if (this.guess[i][j] === max) this.guess[i][j] = 0;
                    } else if (!this.hint[i][j]) {
                        this.guess[i][j] -= 1;
                        if (this.guess[i][j] === -1) this.guess[i][j] = max - 1;
                    }
                }
            }
        }
    ...
    validate() {
        for(let i = 0; i < this.size; i++)for(let j = 0; j < this.size; j++){
            if (this.grid[i][j] != this.guess[i][j]) return false;
        }
        return true;
    }
}

//# sourceMappingURL=index.120579d3.js.map
