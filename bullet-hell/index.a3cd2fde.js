...
MODEL.ship.basic = function(isPlayer) {
    ...
    let yOff = 0;
    if (isPlayer) {
        if (this.vel.x > 0) xOff = -2;
        else if (this.vel.x < 0) xOff = 2;
        if (this.vel.y > 0) yOff = -4;
        else if (this.vel.y < 0) yOff = 4;
    }
    fill("#E74C3C");
    noStroke();
...
MODEL.ship.turret = function(isPlayer) {
    ...
    if (a > -180 && a < 0) rotate(a);
    else if (a >= 90) rotate(180);
    // Barrel
    fill("#ACBAC9");
    rect(-26, 0, 10, 4);
    // Connector
...
//# sourceMappingURL=index.a3cd2fde.js.map
