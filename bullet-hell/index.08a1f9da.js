class Boss extends Ship {
    ...
    act() {
        if (!paused) {
            if (this.stage && this.stages[this.stage].ai) this.stages[this.stage].ai(this);
            this.collidePlayer();
            this.spawnItem();
        }
        super.act();
    ...
    fire() {
        ...
        if (this.stage in this.stages && this.stages[this.stage].attack) this.stages[this.stage].attack(this);
    }
    ...
    switchStage() {
        ...
        this.nextStageTime = -1;
        // Finish stage
        if (this.stage && this.stages[this.stage] && this.stages[this.stage].finish) this.stages[this.stage].finish(this);
        ...
        if (this.nextStage in this.stages) {
            ...
            // Set next stage
            if (Array.isArray(curStage.nextStage)) // Pick random stage from array
            this.nextStage = random(curStage.nextStage);
            else this.nextStage = curStage.nextStage;
            // Set limits for switching stage
            let t = curStage.timeLimit;
            this.nextStageTime = t ? t : -1;
            let h = curStage.healthLimit;
            if (h) this.healthCooldown = h * this.maxHp;
        } else this.stage = null;
    }
}

//# sourceMappingURL=index.08a1f9da.js.map
